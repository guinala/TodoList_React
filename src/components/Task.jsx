...
}) 
{
  ...
  return (
    <li className={`flex items-center gap-3 border rounded-xl px-4 py-3 shadow text-left text-sm transition-colors ${
      isCompleted
        ? 'bg-emerald-950 border-emerald-700 text-emerald-300'
        : 'bg-slate-800 border-slate-600 text-white'
    }`}>
      <button onClick={onClick} className="flex-shrink-0 p-0 bg-transparent border-0 cursor-pointer">
        <span className={`todo__check ${isCompleted ? 'todo__check--checked' : ''}`}>
          {isCompleted ? '✓' : ''}
        </span>
      </button>

      <span className={`flex-1 ${isCompleted ? 'line-through opacity-70' : ''}`}>
        {children}
      </span>
